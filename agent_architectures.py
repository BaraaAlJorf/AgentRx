...
class MedicalMetaScaffolding:
    ...
    def run_meta_loop(self):
        ...
        system_text = (
            f"{STANDARD_SYS_TEXT} Your goal is to determine if the patient will die in the ICU.\n\n"
            f"--- PATIENT SUMMARY ---\n{ps}\n\n"
            f"--- AVAILABLE TOOLS ---\n"
...
def generate_response(prompts, model, processor, max_tokens, **generation_kwargs):
    ...
    try:
        ...
        if "out of memory" in msg or "cuda error" in msg:
            ...
            mid = getattr(model, "model_id", "unknown_model")
            print(f"❌ OOM in generate_response | arch={arch} | model_id={mid}")
            # Optional: clean up before crashing
            torch.cuda.empty_cache()
            gc.collect()
            # Hard fail so you can see which architecture died
            raise
        raise
...
def _build_prompt_content(patient_data, ehr_text, allowed_modalities, is_training_example=False, outcome=None, prompt_type="standard", previous_reasoning=None, feedback = None):
    ...
    if prompt_type == "data_only":
        ...
        prompt_text = (
            ...
            "Identify key risk factors for imminent mortality. Keep the reasoning within 2-4 sentences.\n"
            "Reasoning:"
        )
...
def _load_rulebooks(rulebook_dir):
    """
    Loads the structured JSON rulebook.
    Returns:
        narrative_rules: (List) Now just a placeholder or empty list since it's removed from training.
        diagnostic_rules: (Dict) The structured JSON containing 'ehr', 'cxr', etc.
    """
    diagnostic_rules = {
        'ehr': [], 'cxr': [], 'rr': [], 'ps': [], 'multimodal': []
    }
    ...
    if os.path.exists(d_path):
        try:
            ...
            print(f"Error loading diagnostic rules: {e}")
            
    # Narrative rules were removed from the new RulebookLearner, 
    # so we return an empty list or a default instruction.
    return ["Summarize clinical data accurately."], diagnostic_rules
...
def _run_majority_vote_uni_modal_batch(batch, model, processor, args):
    ...
    for mod_key, mod_name, sys_desc in configs:
        if mod_key in allowed_mods:
            ...
            per_modality_probs[mod_name] = probs
    ...
    for i in range(batch_size):
        ...
        for mod_name in active_voters:
            prob = float(per_modality_probs[mod_name][i])
            text = "YES" if prob >= 0.5 else "NO"
            print(f"[{mod_name}] Vote: {text} | Prob: {prob:.4f}")

        final_prob = float(avg_probs[i])
        final_text = "YES" if final_prob >= 0.5 else "NO"
        print(f"[FINAL] Vote: {final_text} | Prob: {final_prob:.4f}")
...
def _run_debate_unimodal_batch(batch, model, processor, args):
    ...
    3. If No Consensus -> Generate 'Reasoning' -> Pass to Peers -> Repeat.
    """
    allowed_mods = _parse_allowed_modalities(args)
    ...
    potential_agents = [
        ('ehr', 'EHR Specialist', ['ehr'], "You are an Electronic Health Record Specialist. This patient was just admitted. Analyze the vitals and labs."),
        ('cxr', 'CXR Specialist', ['cxr'], "You are a Chest X-ray Specialist. This patient was just admitted. Analyze the image."),
        ('ps', 'Patient Summary Specialist', ['ps'], "You are a Clinical Historian. This patient was just admitted. Analyze the patient summary."),
        ('rr', 'Radiology Report Specialist', ['rr'], "You are a Radiology Report Specialist. This patient was just admitted. Analyze the text report.")
    ]
        ...
        for k, agent in enumerate(agents):
            ...
            for i in batch_indices:
                ...
                txt = (
                    f"{agent['sys']}\n"
                    f"--- TASK ---\n"
                    f"Does this patient die in the ICU?\n"
    ...
    for r in range(n_rounds):
        ...
        for k in range(n_agents):
            ...
            for i in batch_indices:
                ...
                txt = (
                    f"You are expert ICU risk prediction model {k+1}. This patient was just admitted.\n"
                    f"--- TASK ---\n"
                    f"Does this patient die in the ICU?\n"
                    f"Below are your peers' opinions.\n"
                    f"{peer_txt}\n"
...
def _run_mad_agent_batch(batch, model, processor, args):
    ...
    meta_prompt = (
        ...
        "Debate Topic: Does this patient die in the ICU?"
    )
    ...
    for i in range(batch_size):
        ...
        txt = (
            ...
            "Does the patient die in the ICU? Answer only Yes or No.\n"
            "Answer:"
        )
...
def _run_traj_coa_agent_batch(batch, model, processor, args):
    ...
    if 'ehr' in allowed_mods:
        ...
        current_memories = ["Patient admitted to ICU. Initial observation."] * batch_size
...
def _run_agenticds_batch(batch, model, processor, args):
    ...
    Restructured to place multimodal content between expert analysis and the final question.
    """
    ...
    for mod in ['ehr', 'cxr', 'rr', 'ps']:
        ...
        for r in mod_rule_list:
            ...
            rule_text = r.get("Rules", "No specific rule")
        ...
        for i, p in enumerate(batch):
            ...
            if is_available:
                ...
                # --- EXPERT SANDWICH: Rules -> Data -> Risk Assessment ---
                # expert_prefix = (
                #     f"You are an expert specialist in {full_name}.\n"
                #     f"This patient was just admitted to the ICU. "
                #     f"--- RAW DATA CONTEXT ---"
                # )
                
                # expert_suffix = (
                #     f"--- END DATA ---\n\n"
                #     f"You are given these diagnostic rules:\n{mr_str}\n\n"
                #     f"Given the data, diagnostic rules, and using your expertise, clearly outline what level of risk (high, moderate, or low) this patient has for mortality within the ICU."
                #     f"Justify your choice using the rules above. Your assessment and justification should be within a maximum of 100 words. Your assessment should start with 'I suspect this patient will/will not die during their ICU stay.'"
                # )
                
                expert_prefix = (
                    f"You are a specialized critical care expert in {full_name}. "
                    f"A patient has been admitted to the ICU. Analyze the following clinical data carefully.\n"
                    f"--- RAW DATA CONTEXT ---"
                )
                ...
                expert_suffix = (
                    ...
                    f"--- DIAGNOSTIC RULES ---\n{mr_str}\n\n"
                    f"--- TASK ---\n"
                    f"Based on the data and diagnostic rules above, determine if this patient is likely to survive the ICU stay, acknowledging that they are currently receiving treatment and that most patients in the ICU ward survive. Make sure to consider all low, moderate, and high risk factors and to analyze patient trends.\n\n"
                    f"--- REQUIRED OUTPUT FORMAT ---\n"
                    f"You must strictly output only two sentences in the following format:\n"
                    f"'I predict this patient will [survive/succumb]. [One brief sentence explaining the risk level and rule violations].'\n\n"
                    f"Example 1: I predict this patient will survive despite the high risk. Multiple rules are broken but the ICU resources are sufficient.\n"
                    f"Example 2: I predict this patient will succumb. The high risk conditions are too numerous to resolve.\n"
                    f"Do not add introductions, disclaimers, or bullet points."
                )
                
                content, _ = _build_prompt_content(p, p.get('ehr_text', ''), [mod], prompt_type="data_only")
                
                # Combine as: Prefix Text + Multimodal Data + Suffix Text
                full_expert_content = (
                    [{"type": "text", "text": expert_prefix}] + 
                    content + 
                    [{"type": "text", "text": expert_suffix}]
                )
                prompts.append([{"role": "user", "content": full_expert_content}])
                #print(prompts)

        if prompts:
            outputs = generate_response(prompts, model, processor, max_tokens=500)
            for idx, i in enumerate(active_indices):
                expert_analyses[i] += f"[{full_name} Expert Report]:\n{outputs[idx]}\n\n"
    

    # 2. Multimodal Judge Synthesis
    ...
    for i, p in enumerate(batch):
        ...
        # Requirement 1: Break the judge prompt into a prefix and a suffix
        sys_judge_prefix = (
        ...
        sys_judge_suffix = (
            ...
            f"\n--- DECISION ---\n"
            #f"Based on the specialist reports, the raw data provided above, and your expertise, does this patient die in the ICU? "
            f"Based on the specialist reports and your expertise, does this patient die in the ICU? "
            f"Answer only using one word - Yes or No\n"
            f"Answer: "
        )
        
        # active_mods = list(modality_usage_tracking[i])
        # # Fetch the multimodal content (images, etc.)
        # content_judge, _ = _build_prompt_content(p, p.get('ehr_text', ''), allowed_mods, prompt_type="data_only")
        
        # Requirement 1: Construct content with data sandwiched in the middle
        judge_full_content = (
            [{"type": "text", "text": sys_judge_prefix}] + 
            #content_judge + 
            [{"type": "text", "text": sys_judge_suffix}]
        )
        
        final_prompts.append([{"role": "user", "content": judge_full_content}])

    # Batch calculation of probabilities
    texts, probs = generate_yes_no_probability(final_prompts, model, processor, max_tokens=1)
    
    print('Texts:',texts)
    print('Probs:',probs)
    print('Experts:',expert_analyses)
...
def initialize_agent_setup(batch, args):
    ...
    elif agent_setup_name == 'AgentiCDSsimple':
        return _run_agenticds_simplified_batch(batch, model, processor, args)
    else:
