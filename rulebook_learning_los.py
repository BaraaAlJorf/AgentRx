    ...
    def _audit_and_update(self, agent_name, current_rules, full_prompt_content, ground_truth, task_type):
        ...
        if agent_name.lower().startswith("judge") or "judge" in agent_name.lower():
            ...
            prompt = (
                ...
                "- \"Rule\": a short if–then style statement explaining how the multimodal pattern signals the outcome.\n\n"
            ...
            prompt = (
                f"Given this patient data, and the patient's outcome. Please suggest additional rules to augment the attached rulebook of the provided modality.\n"
                f"Do NOT remove or modify existing rules — only add new ones if they are needed to explain the outcome.\n"
                f"--- PATIENT DATA ---\n{context_str}\n\n"
                f"--- CASE OUTCOME: {ground_truth} ---\n\n"
                f"--- CURRENT RULES (as JSON) ---\n{rules_json_str}\n\n"
                f"--- INSTRUCTIONS ---\n"
                f"1) If no additional rules are needed to infer the patient outcome given the attached patient data, output only the token: NO_CHANGE\n"
                f"2) If additional rules would help, output a JSON array (list) of rule objects where each object has exactly these keys:\n"
                f'   "Feature of Interest", "Description", "Patterns", "Rule"\n'
                f'   - "Feature of Interest": short feature name relevant to the attached modality\n'
                f'   - "Description": short description of that feature\n'
                f'   - "Patterns": how to detect the feature or threshold\n'
                f'   - "Rule": short instruction (e.g., "If feature > 4, there is a higher chance for an ICU stay longer than 7 days")\n'
                f"3) Output only valid JSON (an array). No extra commentary or surrounding text. Keep entries concise.\n"
                f"4) Avoid duplicates and empty fields.\n"
                "Output: "
            )
        ...
        def normalize_rule(r):
            ...
            rule_text = (r.get("Rule") or r.get("rule") or "").strip().lower()
        ...
        try:
            ...
            for obj in parsed:
                ...
                validated_obj = {
                    ...
                    "Rule": str(obj.get("Rule", obj.get("rule", "") or "")).strip()
                }
                # Minimal acceptance: must have either Patterns, Feature, or Rule
                if any(validated_obj[k] for k in ["Feature of Interest", "Patterns", "Rule"]):
            ...
            for r in new_rules:
                for k in r:
                    if isinstance(r[k], str):
                        r[k] = r[k].strip()
                if (r.get("Patterns") and r["Patterns"].strip()) or (r.get("Rule") and r["Rule"].strip()) or (r.get("Feature of Interest") and r["Feature of Interest"].strip()):
        ...
        for cand in parsed_new_rules:
            ...
            if sig in existing_sigs:
                print(f"   [DEBUG] Skipping duplicate rule (Feature/Rule/Patterns signature matched): {cand.get('Rule') or cand.get('Patterns') or cand.get('Feature of Interest')}")
                continue
            ...
            cand_clean = {
                ...
                "Rule": cand.get("Rule", "").strip()
            }
            # Re-check minimal acceptance
            if any(cand_clean[k] for k in ["Feature of Interest", "Patterns", "Rule"]):
    ...
    def train_epoch(self, train_loader):
        ...
        for batch in tqdm(train_loader, desc="Auditing Rules"):
            for patient in batch:
                gt_outcome = "patient had an ICU stay >= 7 days" if patient['labels']['los_7'] == 1 else "patient had an ICU stay < 7 days"
