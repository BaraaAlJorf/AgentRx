...
def evaluate_predictions(all_results: list, output_csv_path: str = None) -> dict:
    """
    MODIFIED: Calculates performance metrics for mortality (explicit and implicit)
    and phenotyping tasks, analyzes modality usage, and saves detailed results.
    """
    # --- Prepare lists for sklearn metrics ---
    y_true_mortality, y_pred_mortality = [], []
    y_scores_mortality = [] # For explicit probability
    y_scores_mortality_implicit = [] # NEW: For implicit probability
    ...
    phenotype_labels = list(all_results[0]['ground_truth'].keys())
    phenotype_labels.remove('in_hospital_mortality_48hr')
    ...
    for res in all_results:
        ...
        # Get all three new probability/text fields
        prob_mortality = res['predictions'].get('mortality_probability', 0.5)
        prob_mortality_implicit = res['predictions'].get('mortality_probability_implicit', 0.5)
        ...
        y_scores_mortality.append(prob_mortality) # Explicit
        y_scores_mortality_implicit.append(prob_mortality_implicit) # Implicit

        gt_pheno = [res['ground_truth'][label] for label in phenotype_labels]
        pred_pheno = [res['predictions']['phenotypes'][label] for label in phenotype_labels]

        y_true_pheno.append(gt_pheno)
        y_pred_pheno.append(pred_pheno)
        ...
        if output_csv_path:
            row = {
                ...
                'probability_mortality_implicit': prob_mortality_implicit, # NEW
                'probability_mortality_text': text_mortality # NEW
            }
            for i, label in enumerate(phenotype_labels):
                row[f'gt_{label}'] = gt_pheno[i]
                row[f'pred_{label}'] = pred_pheno[i]
    ...
    # Calculate and add AUROC/AUPRC for EXPLICIT scores
    try:
    ...
    # NEW: Calculate and add AUROC/AUPRC for IMPLICIT scores
    try:
        mortality_auroc_implicit = roc_auc_score(y_true_mortality, y_scores_mortality_implicit)
    except ValueError as e:
        print(f"[Warning] Could not calculate IMPLICIT AUROC: {e}")
        mortality_auroc_implicit = None
    mortality_metrics['auroc_implicit'] = mortality_auroc_implicit

    try:
        mortality_auprc_implicit = average_precision_score(y_true_mortality, y_scores_mortality_implicit)
    except ValueError as e:
        print(f"[Warning] Could not calculate IMPLICIT AUPRC: {e}")
        mortality_auprc_implicit = None
    mortality_metrics['auprc_implicit'] = mortality_auprc_implicit


    # --- 3. Calculate Phenotyping Metrics ---
    phenotyping_metrics = classification_report(
        y_true_pheno, y_pred_pheno, 
        target_names=phenotype_labels, 
        output_dict=True,
        zero_division=0
    )
    
    # --- 4. Calculate Modality Usage Metrics ---
    ...
    # --- 5. Combine all metrics into a final dictionary ---
    final_metrics = {
        "in_hospital_mortality_metrics": mortality_metrics,
        "phenotyping_metrics": phenotyping_metrics,
